...
// At ~45 Hz per anchor these prints saturate the 115200-baud UART and add
// latency jitter to the ranging loop. Keep at 0 for normal operation.
#define DEBUG_RANGING 0

// --- Fixed-rate publish interval (milliseconds) ---
#define PUBLISH_RATE_MS 100   // 10 Hz; change to taste
...
#define IMU_PUBLISH_RATE_MS 20

static SemaphoreHandle_t anchor_mutex = NULL;
...
public:
  ...
  static bool ds_sendFrame(int stage);
  static bool ds_sendRTInfo(int t_roundB, int t_replyB);
...
static int anchor_consec_fails[NUM_ANCHORS] = {0};
#define MAX_TX_FAILS          3
#define MAX_REINITS           5
// After this many consecutive failures (timeouts + TX errors combined) on one
// anchor, skip it and move to the next one. It will be retried on the next cycle.
#define MAX_ANCHOR_CONSEC_FAILS 3
...
// Full chip reinit when the DWM3000 TX engine locks up.
// Uses a physical hard-reset (RST pin) instead of softReset so the
// DWM3000 hardware state is fully cleared without risking an ESP32 crash.
void reinitRadio()
{
  reinit_count++;
  if (reinit_count >= MAX_REINITS)
  {
    Serial.println("[ERROR] DWM3000 unrecoverable after repeated reinits. Restarting ESP32...");
    delay(200);
    esp_restart();
  }

  Serial.print("[WARN] Reinitialising DWM3000 radio (attempt ");
  Serial.print(reinit_count);
  Serial.println(")...");

  // Physical RST pin reset — clears chip hardware state fully.
  DWM3000.hardReset();
  delay(200);

  // Wait for the chip to report IDLE before proceeding.
  unsigned long t0 = millis();
  while (!DWM3000.checkForIDLE())
  {
    if (millis() - t0 > 2000)
    {
      Serial.println("[ERROR] DWM3000 did not reach IDLE after hard reset.");
      break;
    }
    delay(50);
  }

  DWM3000.init();       // init() runs its own internal softReset
  DWM3000.setupGPIO();
  DWM3000.setTXAntennaDelay(ANTENNA_DELAY);
  DWM3000.setSenderID(TAG_ID);
  DWM3000.clearSystemStatus();
  DWM3000.standardRX();
  delay(50);            // let the RX engine settle before the first TX
  tx_fail_count = 0;
  curr_stage = 0;
  current_anchor_index = 0;  // restart from A1 — don't loop forever on the dead anchor
  Serial.println("[INFO] Radio reinit complete.");
}
...
void printAllDistances()
{
  ...
  for (int i = 0; i < NUM_ANCHORS; i++)
  {
    ...
    if (distances[i] > 0)
    {
      DWM3000.printDouble(distances[i], 100, false);
      Serial.print(" cm");
    }
    else
    {
...
void fillImuMessage(const sensors_event_t &accel, const sensors_event_t &gyro, int64_t stamp_ms)
{
  imu_msg.header.stamp.sec = (int32_t)(stamp_ms / 1000);
  imu_msg.header.stamp.nanosec = (uint32_t)((stamp_ms % 1000) * 1000000ULL);

  // MPU6050 library already provides SI units: m/s^2 and rad/s.
...
// ---------------------------------------------------------------------------
// publishDistances()
// Copies the latest filtered distances into the ROS message and publishes.
// Called on a fixed timer, independently of the ranging state machine.
// Anchors that have not yet completed a ranging cycle will have
// filtered_distance == 0.0, which is published as-is so subscribers always
// receive a message at the expected rate.
// ---------------------------------------------------------------------------
...
bool DWM3000Class::ds_sendFrame(int stage)
{
  setMode(1);
  ...
  setFrameLength(4);
  writeFastCommand(0x00); // TRXOFF — force chip to IDLE before TX
  delayMicroseconds(20); // let the state machine settle to IDLE
  clearSystemStatus();
  TXInstantRX();
  // Widened to 20ms to absorb WiFi/micro-ROS scheduling jitter — UWB
  // air-time is ~60us so this costs nothing in the success case.
...
bool DWM3000Class::ds_sendRTInfo(int t_roundB, int t_replyB)
{
  setMode(1);
  ...
  while (millis() < deadline)
  {
    ...
    // no delayMicroseconds() here — busy-poll for lowest latency on
    // success, since we WANT to catch the flag the instant it's set
  }

  Serial.println("[ERROR] Could not send RT info successfully!");
  return false;
}
...
void publishTask(void *pvParameters) {
  unsigned long local_last_dist = millis();
  unsigned long local_last_imu  = millis();
  while (1) {
...
void setup()
{
  Serial.begin(115200);
  delay(2000);
  Serial.println("\n\n=== UWB TAG + micro-ROS WiFi DEBUG START ===");
  delay(3000);   // TEMP: extra pause so serial monitor is definitely attached
  Wire.begin(21, 22);
  ...
  Serial.println("[DEBUG] MPU6050 OK");
  initializeAnchors();
  Serial.print("[DEBUG] Initialized ");
  Serial.print(NUM_ANCHORS);
  Serial.println(" anchors");
  Serial.println("[DEBUG] Starting WiFi connection to Nautilus...");
  Serial.print(" SSID: ");
  Serial.println(ssid);
  ...
  else
  {
    ...
    Serial.println(" → Check hotspot password, channel, and that the ESP32 is in range.");
    while (1)
      ;
  }
  WiFi.setSleep(false);
  Serial.println("[DEBUG] Setting up micro-ROS WiFi transport...");
  set_microros_wifi_transports(ssid, password, agent_ip, agent_port);
  delay(1000);
  WiFi.setSleep(false);   // re-assert after transport setup
  Serial.println("[DEBUG] Creating micro-ROS structures...");
  allocator = rcl_get_default_allocator();
  rcl_ret_t ret = rclc_support_init(&support, 0, NULL, &allocator);
  Serial.print(" rclc_support_init → ");
  Serial.println(ret == RCL_RET_OK ? "OK ✓" : "FAILED");
  ret = rclc_node_init_default(&node, "uwb_tag", "", &support);
  ...
  ret = rclc_publisher_init_default(
      ...
      "/uwb/distances");
  Serial.print(" rclc_publisher_init_default → ");
  Serial.println(ret == RCL_RET_OK ? "OK ✓" : "FAILED");
  std_msgs__msg__Float32MultiArray__init(&distances_msg);
  ...
  ret = rclc_publisher_init_default(
      ...
      "/imu/data");
  Serial.print(" imu_publisher → ");
  Serial.println(ret == RCL_RET_OK ? "OK ✓" : "FAILED");

  sensor_msgs__msg__Imu__init(&imu_msg);
  rosidl_runtime_c__String__assign(&imu_msg.header.frame_id, IMU_FRAME_ID);

  // No orientation estimate is produced here; advertise that explicitly.
  ...
  Serial.println("[DEBUG] UWB hardware init starting...");
  DWM3000.begin();
  DWM3000.hardReset();
  delay(200);
  if (!DWM3000.checkSPI())
  {
    Serial.println("[ERROR] SPI connection to DWM3000 failed!");
    while (1)
      ;
  }
  while (!DWM3000.checkForIDLE())
  ...
  anchor_mutex = xSemaphoreCreateMutex();
  xTaskCreatePinnedToCore(publishTask, "PubTask", 8192, NULL, 1, NULL, 0);

  Serial.println("=== SETUP FINISHED ===");
  Serial.println("> TAG ready - waiting for anchors...\n");
}
void loop()
{
...
#if DEBUG_RANGING
    Serial.print("[RANGING] Sending Poll to Anchor ");
    Serial.println(currentAnchorId);
#endif
    if (!DWM3000.ds_sendFrame(1))
    {
      tx_fail_count++;
      anchor_consec_fails[current_anchor_index]++;

      // Lightest response: just clear status and retry RX, no reinit yet.
      // This handles transient WiFi-jitter-induced TX misses without
      // disrupting the radio state for other anchors.
      recoverRadioRx();

      if (anchor_consec_fails[current_anchor_index] >= MAX_ANCHOR_CONSEC_FAILS)
      {
        // This anchor specifically is not responding at all across many
        // attempts — skip it for this rotation so A2/A3 keep updating.
        Serial.print("[WARN] Anchor ");
        Serial.print(currentAnchorId);
        Serial.println(" unresponsive, skipping for this cycle.");
        anchor_consec_fails[current_anchor_index] = 0;
        switchToNextAnchor();
        curr_stage = 0;
      }
      else if (tx_fail_count >= MAX_TX_FAILS)
      {
        // Only escalate to a full chip reinit if failures are happening
        // broadly (across anchors), not just on one — this is the signal
        // that the radio itself, not just one link, is in a bad state.
        reinitRadio();
      }
      // else: just loop back to case 0 and retry the SAME anchor —
      // most transient WiFi-jitter misses succeed on the very next attempt.
      break;
    }
    tx_fail_count = 0;
    reinit_count = 0;
...
#if DEBUG_RANGING
    Serial.print(" → TX timestamp: ");
    Serial.println(currentAnchor->tx);
#endif
    currentAnchor->stage_timer = millis();
    curr_stage = 1;
    break;
  case 1:
    ...
    {
#if DEBUG_RANGING
      Serial.print("[RANGING] Received something from Anchor ");
      Serial.println(currentAnchorId);
#endif
      DWM3000.clearSystemStatus();
      if (rx_status == 1)
    ...
    else if (millis() - currentAnchor->stage_timer > STAGE_TIMEOUT_MS)
    {
  ...
  case 3:
    ...
    {
      Serial.print("[TIMEOUT] No RT info from Anchor ");
      Serial.println(currentAnchorId);
      anchor_consec_fails[current_anchor_index]++;
      recoverRadioRx();
      curr_stage = 0;
