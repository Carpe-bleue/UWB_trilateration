...
static int ANTENNA_DELAY = 16385;
int led_status = 0;
int destination = 0x0;
int sender = 0x0;
int config[] = {
...
void publishDistances() {
    ...
    for (int i = 0; i < NUM_ANCHORS; i++) {
        ...
        if (anchors[i].filtered_distance <= 0) has_valid = false;
    }

    rcl_ret_t pub_ret = rcl_publish(&publisher, &distances_msg, NULL);
    if (pub_ret == RCL_RET_OK) {
        if (has_valid) {
            Serial.println("[DEBUG] Published valid distances to /uwb/distances");
        } else {
            Serial.println("[DEBUG] Published (some zeros still)");
        }
    } else {
...
void setup()
{
  ...
  last_publish_time = millis();   // initialise publish timer
...
{
  // -------------------------------------------------------------------------
  // Fixed-rate publish: fire every PUBLISH_RATE_MS regardless of ranging state
  // -------------------------------------------------------------------------
  unsigned long now = millis();
  if (now - last_publish_time >= PUBLISH_RATE_MS)
  {
  ...
  // -------------------------------------------------------------------------
  // IMU publish (every loop iteration, same as before)
  // -------------------------------------------------------------------------
  ...
  imu_data[0] = a.acceleration.x;
  imu_data[1] = a.acceleration.y;
  imu_data[2] = a.acceleration.z;
  imu_data[3] = g.gyro.x;
  imu_data[4] = g.gyro.y;
  imu_data[5] = g.gyro.z;
  ...
  // -------------------------------------------------------------------------
  // UWB ranging state machine (unchanged)
  // -------------------------------------------------------------------------
  switch (curr_stage)
  {
  case 0:
  ...
  case 4:
  {
    ...
    printAllDistances();
    // Distance array is updated by publishDistances() on the timer above;
    // no per-ranging publish needed here anymore.
    switchToNextAnchor();
    curr_stage = 0;
    break;
  }
